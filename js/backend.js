...
function outputColumns(paramArr){
	...
	if (outputList.length == 0){
		alert("That medicine can't be found. Please ensure it is spelled correctly");
	} else {
		outputList.sort();
		for (x = 0; x < outputList.length; x++){
			try{
				if (outputList[x] == outputList[x+1]){
					var index = outputList.indexOf(outputList[x+1]);
					console.log(outputList[x+1] + " removed");
					outputList.splice(index, 1);
				}
				else if (outputList[x] == outputList[x-1]) {
					var index = outputList.indexOf(outputList[x-1]);
					console.log(outputList[x-1] + " removed");
					outputList.splice(index, 1);
				}
			} 																														//TODO: Ensure this doesn't remove wrong medicines
			catch(err){
				break;
			}
		}
...
function columnSearch(toFind){
	...
	for (x = 0; x < colArr.length; x++) {
		for (y = 0; y < colArr[x].length; y++){
			if (colArr[x][y] == toFind) {
				itemColumns.push(colNameArr[x]);
			}
		}
	}
...
function validateInput(toValidate){
	...
	for (x = 0; x < toValidate.length; x++){
		if (allowedChars.indexOf(toValidate[x]) == -1){
			inputIsValid = false;
			break;
		}
	}
